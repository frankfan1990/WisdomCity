...
#pragma mark 我的 -  四个VIew的第三个 - 报修记录 -  点击报修 

#define WORDCOUNT 500
@interface RZAddRepairViewController ()
{
...
@implementation RZAddRepairViewController
...
- (void)viewDidLoad
{
    ...
    {
        txtName.placeholder=@"修什么？";
        txtAddress.placeholder=@"期望上门时间段";
        txtInfo.text=@"详细描述...";
...
-(void)textViewDidBeginEditing:(UITextView *)textView{
    if([textView.text rangeOfString:@"详细描述"].location!=NSNotFound){
        textView.text=@"";
    }
}
...
-(void)textViewDidEndEditing:(UITextView *)textView{
    if([txtInfo.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]].length<1||[txtInfo.text  rangeOfString:@"详细描述"].location!= NSNotFound){
        txtInfo.textColor=UIColorFromRGB(0xcbcbcb);
        txtInfo.text=@"详细描述...";
        lbwordcount.text=[NSString stringWithFormat:@"%d字",WORDCOUNT];
        lbwordcount.textColor=UIColorFromRGB(0xcbcbcb);
...
-(BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    if([text isEqualToString:@""]&&[txtInfo.text  rangeOfString:@"详细描述"].location!= NSNotFound){
        return YES;
    }
